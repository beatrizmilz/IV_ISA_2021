...
meta() %>%
  ...
  meta_social(
    title = "Transparência em atas de Comitês de Bacias Hidrográficas na MMP",
    description = paste(
      "Avaliação da transparência em atas de reuniões ",
      "dos Comitês de Bacias Hidrográficas na ",
      "Macrometrópole Paulista (Brasil).",
      "Apresentado por Beatriz Milz",
      "Na Conferencia Latinoamericana sobre Uso de R en Investigación e Desarrollo (LatinR)."
      
    ),
    url = "https://macroamb-latin-r.netlify.app/",
    image = "https://macroamb-latin-r.netlify.app/img/share-card.png",
    image_alt = paste(
...
`r icon::fa("angle-right")` Mais de 170 municípios (abrangendo a Região Metropolitana de São Paulo)

`r icon::fa("angle-right")` Mais de 33 milhões de habitantes (em 2018) ([EMPLASA, 2019](https://emplasa.sp.gov.br/MMP))

`r icon::fa("angle-right")` Maior aglomeração urbana do Brasil ([DAEE, 2013](http://www.daee.sp.gov.br/index.php?option=com_content&view=article&id=1112:plano-diretor-de-aproveitamento-dos-recursos-hidricos-para-a-macrometropole-paulista); [TAVARES, 2018](https://scielo.conicyt.cl/scielo.php?script=sci_arttext&pid=S0250-71612018000300115))
...
- Utilização:
  ...
  - Ambiente de Desenvolvimento Integrado (IDE) [RStudio](https://rstudio.com/)
  
```{r, out.width="30%"}
knitr::include_graphics("img/rstudio-r.jpg")
```

- Diversos pacotes para coleta, limpeza, análise, visualização e comunicação dos dados: `rvest`, `purrr`, `dplyr`, `tibble`, `stringr`, `magrittr`, `glue`, `ggplot2` (que são parte do `tidyverse`), `httr`, `sf`, `ggspatial`.

- __Coleta de dados__: Raspagem de dados de site (Web Scraping) em Setembro/2020
...
Página web: http://www.sigrh.sp.gov.br/cbhpcj/atas

---
### HTML -> Raspagem de dados -> Tabela 
...
# df_comite %>%
#   select(nome_ugrhi,
#          nome_completo_reuniao,
#          data_da_reuniao,
#          data_de_postagem,
#          link_ata_url) %>%
#   arrange(data_da_reuniao) %>%
#   head(6) %>% knitr::kable(caption = ' Dados coletados (exemplo parcial). [Download da tabela completa (setembro/2020) no formato `csv`.](https://macroamb-latin-r.netlify.app/dados/dados_atas_2020-09-16.csv)')
```
